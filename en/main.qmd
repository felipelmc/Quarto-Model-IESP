...
# ---- Nature of the work on the title page ----
# The official English mask keeps this text (and the "Orientador" label) in
# Portuguese; omitting `thesis-nature` lets the class generate the standard
# Portuguese sentence. Set it explicitly only to override.
...
n <- 60
x <- runif(n, 0, 10)
y <- 2 + 0.8 * x + rnorm(n, 0, 1.2)
fit <- lm(y ~ x)            # OLS closed form: (X'X)^{-1} X'y
```

@fig-regression shows a set of points $(x_i, y_i)$ and the line fitted by
@eq-ols. @tbl-coef summarises the estimates, with their standard errors and
$t$-statistics.

```{r}
#| label: tbl-coef
#| tbl-cap: "Ordinary least squares estimates. Source: prepared by the author (2026)."
#| echo: false
s <- summary(fit)$coefficients
fmt <- function(v) formatC(v, format = "f", digits = 2)
tab <- data.frame(
  coef = c("$\\beta_0$ (intercept)", "$\\beta_1$ (slope)"),
  est  = fmt(s[, "Estimate"]),
  se   = fmt(s[, "Std. Error"]),
  tval = fmt(s[, "t value"])
)
knitr::kable(
  tab, booktabs = TRUE, escape = FALSE, align = "lccc", row.names = FALSE,
...
#| fig-cap: "Regression line fitted by ordinary least squares. Source: prepared by the author (2026)."
#| out-width: "75%"
#| fig-width: 6
#| fig-height: 4
#| echo: false
op <- par(mar = c(4, 4, 1, 1), family = "serif")
plot(x, y, pch = 1, xlab = "x", ylab = "y", bty = "l", las = 1)
abline(fit, lwd = 2)
legend("topleft", bty = "n",
       legend = c("Observations", "OLS fit"),
