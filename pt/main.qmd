...
fit <- lm(y ~ x)            # forma fechada do MQO: (X'X)^{-1} X'y
```
...
```{r}
#| label: tbl-coef
#| tbl-cap: "Estimativas de mínimos quadrados ordinários. Fonte: elaboração própria (2026)."
#| echo: false
s <- summary(fit)$coefficients
fmt <- function(v) formatC(v, format = "f", digits = 2, decimal.mark = ",")
tab <- data.frame(
  coef = c("$\\beta_0$ (intercepto)", "$\\beta_1$ (inclinação)"),
  est  = fmt(s[, "Estimate"]),
  se   = fmt(s[, "Std. Error"]),
  tval = fmt(s[, "t value"])
)
knitr::kable(
  ...
  col.names = c("Coeficiente", "Estimativa", "Erro-padrão", "Estatística $t$")
)
```

```{r}
#| label: fig-regressao
#| fig-cap: "Reta de regressão ajustada por mínimos quadrados ordinários. Fonte: elaboração própria (2026)."
#| out-width: "75%"
#| fig-width: 6
#| fig-height: 4
#| echo: false
...
legend("topleft", bty = "n",
       legend = c("Observações", "Ajuste por MQO"),
